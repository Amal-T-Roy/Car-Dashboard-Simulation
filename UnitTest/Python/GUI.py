...
logging.basicConfig(
    filename="GUI.log",
    level=logging.INFO,
    format="%(asctime)s:%(process)d:%(processName)s:%(funcName)s:%(message)s",
)
...
# Variables to keep track of odo and fuel
...
def IPC():
    while 1:
        ...
        if FuelLevel <= 0:  # If fuel is exhausted
            ...
            # IgnitionButton.configure(state="disabled")
            return
        else:
            FuelDisplay.insert(0, str(FuelLevel) + " l")  # update val
...
def StartButtonPress():
    ...
    if Engine_status == False:
        ...
        SpeedSettingSlider.configure(state="disabled", troughcolor="black")
    # logging.info(f"Engine Status = {Engine_status}")
...
def UpdateSpeed():
    """The dummy variable is given,as the way slider calls a function is by passing a variable to,it
    But if my function takes no parameter,we get "TypeError: slide() takes 0 positional arguments but 1 was given"
    """
...
def ReceiveDataFromServer(sock):
    """
    Receives 3 integer values -> Distance,EngineStatus,FuelLevel
    """
    data = sock.recv(12)  # Receive 3 integers
    Distance, EngineStatus, FuelLevel = struct.unpack(
        "iii", data
    )  # Unpack the bytes into 3 integers
    Values = [Distance, EngineStatus, FuelLevel]
    print("Received 3 integers from socket:", Distance, EngineStatus, FuelLevel)
    # logging.info("Received 3 integers from socket:",
    #              Distance, EngineStatus, FuelLevel)
    return Values
